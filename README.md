...
├── src/                # Source code (safe versions)
│   └── update-metadata.ts  # Metadata update script
...
```bash
git clone https://github.com/xofneb/shipep-token.git
cd shipep-token
```
...
- `npm run update-metadata` - Update token metadata
...
- This repository contains only safe, public information
